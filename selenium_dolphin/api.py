...
class DolphinAPI:
	...
	def get_profiles(self, page=1, limit=50):
		r = self.s.get(f'https://dolphin-anty-api.com/browser_profiles?page={page}&limit={limit}')
		try:
			return r.json()
		except:
	...
	def create_profile(self, data):
	    r = self.s.post('https://dolphin-anty-api.com/browser_profiles', json=data)
	    try:
	        return r.json()
	    except:
	        raise RuntimeError(r.text)
	...
	def edit_profile(self, id, data):
		    r = self.s.patch(f'https://dolphin-anty-api.com/browser_profiles/{id}', json=data)
		    try:
		        return r.json()
		    except:
		        raise RuntimeError(r.text)
	...
	def delete_profiles(self, ids):
	    r = self.s.delete('https://dolphin-anty-api.com/browser_profiles?forceDelete=1', json={'ids':ids})
	    try:
	        return r.json()
	    except:
	        raise RuntimeError(r.text)
	...
	def generate_mac(self):
	    r = self.s.get('https://dolphin-anty-api.com/browser_profiles/generate-mac')
	    try:
	        return r.json()
	    except:
	        raise RuntimeError(r.text)
